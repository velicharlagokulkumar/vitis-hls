...
`define TV_IN_B_TDATA "../tv/cdatafile/c.multiply.autotvin_B.dat"
...
module AESL_axi_s_B (
    ...
    assign TRAN_B_TVALID_temp = ~(B_TDATA_empty);
        ...
        forever begin
            ...
            if (ready == 1) begin
                if (token != "[[[/runtime]]]") begin
                    ...
                    token = read_token(fp);
                    fifo_B_TDATA.snapshot();
                end else begin
                    fifo_B_TDATA.restore();
                    if (fp != 0) begin
                        $fclose(fp);
                        fp = 0;
                    end
